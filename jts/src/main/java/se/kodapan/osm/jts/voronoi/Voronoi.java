 ...
 */
public class Voronoi<ClassType> {

  public static class Instance<ClassType> {
    private Object object;
    private ClassType classValue;
    private Coordinate coordinate;

    public Instance() {
    }

    public Instance(Object object, ClassType classValue, Coordinate coordinate) {
      this.object = object;
    ...
    public void setClassValue(ClassType classValue) {
      this.classValue = classValue;
    }
  ...
  private Logger log = LoggerFactory.getLogger(Voronoi.class);

  private GeometryFactory factory;

  public Voronoi(GeometryFactory factory) {
    this.factory = factory;
  }

  private int numberOfThreads = 16;

  private Map<ClassType, Set<Instance<ClassType>>> coordinatesByClass = new HashMap<ClassType, Set<Instance<ClassType>>>();
  ...
  public Map<ClassType, List<Polygon>> build() throws InterruptedException {
    ...
    for (Map.Entry<ClassType, Set<Instance<ClassType>>> entry : this.coordinatesByClass.entrySet()) {
      Set<Coordinate> values = new HashSet<Coordinate>(entry.getValue().size());
      for (Instance instance : entry.getValue()) {
        values.add(instance.coordinate);
        counter++;
      }
  ...
  public void addCoordinate(ClassType _class, Coordinate coordinate) {
    Set<Instance<ClassType>> instances = coordinatesByClass.get(_class);
    if (instances == null) {
      instances = new HashSet<Instance<ClassType>>();
      coordinatesByClass.put(_class, instances);
    }
    instances.add(new Instance<ClassType>(null, _class, coordinate));
  }
  ...
  public Map<ClassType, Set<Instance<ClassType>>> getCoordinatesByClass() {
    return coordinatesByClass;
  }

  public void setCoordinatesByClass(Map<ClassType, Set<Instance<ClassType>>> coordinatesByClass) {
  ...
  public int countInstances() {
    int counter = 0;
    for (Set<Instance<ClassType>> instances : this.getCoordinatesByClass().values()) {
